...
const RETRIEVAL_VERSION = 'r3';   // r2：加相关度下限 RERANK_MIN，离题问题改为直接拒答；r3：提示词允许小标题与要点列表（配合问法页语录式呈现）
...
async function serveAsk(request, env, url, ctx) {
  ...
  const messages = [
    { role: 'system', content:
      ...
      `六、用平实庄重的白话回答，不堆砌辞藻，不写「根据资料」「综上所述」之类套话，控制在约 ${ANSWER_CHARS} 字以内；\n` +
      // 前端按语录式呈现，小标题渲染成节名、「1. 」渲染成要点 —— 上了年纪的读者靠这些扫读。
      // 「短答不必分节」这一句不能省：没有它，模型会给三句话的答案也硬套两个标题。
      '七、便于扫读：开头一两句总说要旨；篇幅较长时按义理分节，每节冠以「## 」起头的小标题（四到八字，两到四节），节内要点可用「1. 」「2. 」列出；三两句能答完的短答不必分节。' },
    ...hist,
