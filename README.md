...
| **问法** | 独占整屏的请益堂：RAG 检索 8999 块原文 → 流式作答带出处编号，点编号跳读原文；语录体例「问／答」、出处卡、往问（本机近 30 段）、今日一问；另有 969 条问答直接检索 |
...
- 页面（2026-09 重做，[public/js/ask.js](public/js/ask.js) / [public/css/ask.css](public/css/ask.css)）：
  `#view-wenda` 在 `data-view="wenda"` 下 fixed 满屏，底部导航让位，播放中的停靠条移到输入框上方；
  会话卷自己滚（贴底才跟、翻看不打断）；对话存 `fy.chats`（近 30 段、每段 40 条，本机不同步，
  旧键 `fy.chat` 首次读到时并入）；「今日一问」从 library.json 的问答里按北京日期确定性取一条；
  提示词 r3 起允许「## 小标题」与「1. 」要点，渲染层照小标题/列表排；桌面 ≥1120px 右栏列本答出处与往问
...
public/
  ...
  js/ask.js           问法 RAG 对话（往问各段与在途请求随模块，事件层走访问器）
